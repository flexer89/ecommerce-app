...
@router.get(
    ...
    responses={
        200: {"description": "List of products retrieved successfully with pagination and filtering options."},
        404: {"model": ErrorResponse, "description": "No products found with the specified filters"},
    },
...
def read_products(
    ...
    logger.info(f"Request to retrieve products with limit {limit}, offset {offset}, and filters")
...
@router.post(
    ...
    responses={
        ...
        400: {"model": ErrorResponse, "description": "Invalid input data, such as incorrect discount value or missing required fields"},
    },
...
async def download_binary_product_image(product_id: int, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
...
def get_product_trends(db: Session = Depends(get_db)):
    ...
    low_stock_products = db.query(ProductModel).filter(ProductModel.stock < 10).order_by(ProductModel.stock.asc()).all()
    thirty_days_ago = datetime.now() - timedelta(days=30)
    new_products = db.query(ProductModel).filter(ProductModel.created_at >= thirty_days_ago).order_by(ProductModel.created_at.desc()).all()
    discounted_products = db.query(ProductModel).filter(ProductModel.discount > 0).order_by(ProductModel.discount.desc()).all()
    ...
    return {
        ...
        "low_stock_products": [
            {"id": product.id, "name": product.name, "stock": product.stock, "price": product.price, "category": product.category}
            for product in low_stock_products
        ],
        "new_products": [
            {"id": product.id, "name": product.name, "created_at": product.created_at, "price": product.price, "category": product.category}
            for product in new_products
        ],
        "discounted_products": [
            {"id": product.id, "name": product.name, "discount": product.discount, "price": product.price, "category": product.category}
