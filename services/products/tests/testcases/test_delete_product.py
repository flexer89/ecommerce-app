...
def setup_database():
    ...
    db = TestingSessionLocal()
    product = Product(name="Test Product", description="Test Description", price=10.0, stock=10, discount=0.1, category="Test Category")
    db.add(product)
    db.commit()
    db.refresh(product)
    yield product.id
...
def test_delete_product_not_found():
    response = client.delete("/delete/9999")  # Assuming 9999 is a non-existent product ID
