...
def test_db():
    ...
    Base.metadata.create_all(bind=engine)
    db = TestingSessionLocal()
    product1 = Product(name="Test Product 1", description="Test Description 1", price=10.0, stock=10, discount=0.1, category="Test Category", image=None)
    product2 = Product(name="Test Product 2", description="Test Description 2", price=20.0, stock=20, discount=0.2, category="Test Category", image=None)
    db.add(product1)
    db.add(product2)
    db.commit()
    db.refresh(product1)
