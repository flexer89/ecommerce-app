...
def setup_database():
    ...
    db = TestingSessionLocal()
    product1 = Product(id=1, name="Product 1", stock=10, price=100, category="Category 1")
    product2 = Product(id=2, name="Product 2", stock=5, price=200, category="Category 2")
    db.add(product1)
    db.add(product2)
    db.commit()
    db.close()
...
def test_update_quantity_success(setup_database):
    ...
    assert response.json() == {"status": "success", "message": "Quantities updated successfully"}
...
def test_update_quantity_insufficient_stock(setup_database):
    ...
    assert "Insufficient quantity for the following products" in response.json()["detail"]
